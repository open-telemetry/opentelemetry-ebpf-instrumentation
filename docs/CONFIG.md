...
| `channel_send_timeout_panic` | `boolean` | `OTEL_EBPF_CHANNEL_SEND_TIMEOUT_PANIC` |  |  |  |  |
| `enforce_sys_caps` | `boolean` | `OTEL_EBPF_ENFORCE_SYS_CAPS` |  |  |  | Check for required system capabilities and bail if they are not present. If set to 'false', OBI will still print a list of missing capabilities, but the execution will continue |
...
| `profile_port` | `integer` | `OTEL_EBPF_PROFILE_PORT` |  |  |  |  |
...
| `attributes.kubernetes.drop_external` | `boolean` | `OTEL_EBPF_NETWORK_DROP_EXTERNAL` |  |  |  | Will drop, in NetO11y component, any flow where the source or destination IPs are not matched to any kubernetes entity, assuming they are cluster-external |
...
| `attributes.kubernetes.meta_restrict_local_node` | `boolean` | `OTEL_EBPF_KUBE_META_RESTRICT_LOCAL_NODE` |  |  |  | Will download only the metadata from the Pods that are located in the same node as the OBI instance. It will also restrict the Node information to the local node. |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `discovery.bpf_pid_filter_off` | `boolean` | `OTEL_EBPF_BPF_PID_FILTER_OFF` |  |  |  | Debugging only option. Make sure the kernel side doesn't filter any PIDs, force user space filtering. |
| `discovery.default_exclude_instrument` | [`GlobAttributes`](#globattributes)[] |  | `map[cmd_args:map[] containers_only:false exe_path:map[] exports:map[] k8s_pod_annotations:<nil> k8s_pod_labels:<nil> languages:map[] metrics:map[features:0] name: namespace: open_ports:map[Ranges:<nil>] routes:<nil> sampler:<nil> target_pids:<nil>]`, `map[cmd_args:map[] containers_only:false exe_path:map[] exports:map[] k8s_pod_annotations:<nil> k8s_pod_labels:<nil> languages:map[] metrics:map[features:0] name: namespace: open_ports:map[Ranges:<nil>] routes:<nil> sampler:<nil> target_pids:<nil>]` |  |  | By default prevents self-instrumentation of OBI as well as related observability tools It must be set to an empty string or a different value if self-instrumentation is desired. |
| `discovery.default_exclude_services` | [`RegexSelector`](#regexselector)[] |  | `map[cmd_args:map[] containers_only:false exe_path:map[] exe_path_regexp:map[] exports:map[] k8s_pod_annotations:<nil> k8s_pod_labels:<nil> languages:map[] metrics:map[features:0] name: namespace: open_ports:map[Ranges:<nil>] routes:<nil> sampler:<nil> target_pids:<nil>]`, `map[cmd_args:map[] containers_only:false exe_path:map[] exe_path_regexp:map[] exports:map[] k8s_pod_annotations:<nil> k8s_pod_labels:<nil> languages:map[] metrics:map[features:0] name: namespace: open_ports:map[Ranges:<nil>] routes:<nil> sampler:<nil> target_pids:<nil>]` |  | Yes | By default prevents self-instrumentation of OBI as well as related observability tools It must be set to an empty string or a different value if self-instrumentation is desired.  Use DefaultExcludeInstrument instead |
...
| `discovery.exclude_otel_instrumented_services_span_metrics` | `boolean` | `OTEL_EBPF_EXCLUDE_OTEL_INSTRUMENTED_SERVICES_SPAN_METRICS` |  |  |  | Disables generation of span metrics of services which are already instrumented |
...
| `discovery.skip_go_specific_tracers` | `boolean` | `OTEL_EBPF_SKIP_GO_SPECIFIC_TRACERS` |  |  |  | This can be enabled to use generic HTTP tracers only, no Go-specifics will be used: |
...
| `ebpf.bpf_debug` | `boolean` | `OTEL_EBPF_BPF_DEBUG` |  |  |  | Enables logging of eBPF program events |
| `ebpf.bpf_fs_path` | `string` | `OTEL_EBPF_BPF_FS_PATH` | `/sys/fs/bpf/` |  |  | BPF path used to pin eBPF maps |
| `ebpf.context_propagation` | `string` | `OTEL_EBPF_BPF_CONTEXT_PROPAGATION` |  | ``, `all`, `disabled` |  | Enables distributed context propagation. Can be a combination of: headers, tcp (e.g., "headers,tcp" or "all") |
| `ebpf.couchbase_db_cache_size` | `integer` | `OTEL_EBPF_COUCHBASE_DB_CACHE_SIZE` | `1024` |  |  |  |
| `ebpf.disable_black_box_cp` | `boolean` | `OTEL_EBPF_BPF_DISABLE_BLACK_BOX_CP` |  |  |  | Disables OBI black-box context propagation. Used for testing purposes only. |
...
| `ebpf.force_bpf_map_reader` | `string` | `OTEL_EBPF_FORCE_BPF_MAP_READER` |  | `auto`, `batch`, `legacy` |  | Forces the PerCPU HashMap operation of the Network Flows reader. The system will always try "batch", which is more efficient, but legacy systems like RHEL8-based will fallback to "legacy" (the slowest, more resource-consuming iterate&delete approach). |
| `ebpf.heuristic_sql_detect` | `boolean` | `OTEL_EBPF_HEURISTIC_SQL_DETECT` |  |  |  | Enables the heuristic based detection of SQL requests. This can be used to detect talking to databases other than the ones we recognize in OBI, like Postgres and MySQL |
| `ebpf.high_request_volume` | `boolean` | `OTEL_EBPF_BPF_HIGH_REQUEST_VOLUME` |  |  |  | Optimizes for getting requests information immediately when request response is seen |
| `ebpf.http_request_timeout` | `duration` | `OTEL_EBPF_BPF_HTTP_REQUEST_TIMEOUT` | `0s` | `30s`, `5m`, `1ms`, etc |  | Must be at least 0 |
| `ebpf.instrument_cuda` | `integer` | `OTEL_EBPF_INSTRUMENT_CUDA` | `1` |  |  | Enables GPU instrumentation for CUDA kernel launches and allocations |
...
| `ebpf.override_bpfloop_enabled` | `boolean` | `OTEL_EBPF_OVERRIDE_BPF_LOOP_ENABLED` |  |  |  | Skips checking the kernel version for bpf_loop functionality. Some modified kernels have this backported prior to version 5.17. |
...
| `ebpf.protocol_debug_print` | `boolean` | `OTEL_EBPF_PROTOCOL_DEBUG_PRINT` |  |  |  | Enables debug printing of the protocol data |
| `ebpf.track_request_headers` | `boolean` | `OTEL_EBPF_BPF_TRACK_REQUEST_HEADERS` |  |  |  | If enabled, the kprobes based HTTP request tracking will start tracking the request headers to process any 'Traceparent' fields. |
| `ebpf.traffic_control_backend` | `string` | `OTEL_EBPF_BPF_TC_BACKEND` | `3` | `auto`, `tc`, `tcx` |  | Select the TC attachment backend: accepted values are 'tc' (netlink), and 'tcx' |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.buffer_sizes.http` | `integer` | `OTEL_EBPF_BPF_BUFFER_SIZE_HTTP` |  |  |  |  |
| `ebpf.buffer_sizes.kafka` | `integer` | `OTEL_EBPF_BPF_BUFFER_SIZE_KAFKA` |  |  |  |  |
| `ebpf.buffer_sizes.mysql` | `integer` | `OTEL_EBPF_BPF_BUFFER_SIZE_MYSQL` |  |  |  |  |
| `ebpf.buffer_sizes.postgres` | `integer` | `OTEL_EBPF_BPF_BUFFER_SIZE_POSTGRES` |  |  |  |  |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.maps_config.global_scale_factor` | `integer` |  |  |  |  | Scales map sizes in powers of two:   > 0: grows size (2x per step)   < 0: shrinks size (1/2 per step)   = 0: no change |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.aws.enabled` | `boolean` | `OTEL_EBPF_HTTP_AWS_ENABLED` |  |  |  | Enable AWS services (S3, SQS, etc.) payload extraction and parsing |

#### `ebpf.payload_extraction.http.elasticsearch`

| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.elasticsearch.enabled` | `boolean` | `OTEL_EBPF_HTTP_ELASTICSEARCH_ENABLED` |  |  |  | Enable Elasticsearch payload extraction and parsing |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.enrichment.enabled` | `boolean` | `OTEL_EBPF_HTTP_ENRICHMENT_ENABLED` |  |  |  | Enable HTTP header and payload enrichment |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.enrichment.policy.default_action` | `string` | `OTEL_EBPF_HTTP_ENRICHMENT_DEFAULT_ACTION` | `2` | `exclude`, `include`, `obfuscate` |  | Specifies what to do when no rule matches: "include" or "exclude" |
| `ebpf.payload_extraction.http.enrichment.policy.match_order` | `string` | `OTEL_EBPF_HTTP_ENRICHMENT_MATCH_ORDER` | `1` | `first_match_wins` |  | Controls how rules are evaluated: "first_match_wins" |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.genai.anthropic.enabled` | `boolean` | `OTEL_EBPF_HTTP_ANTHROPIC_ENABLED` |  |  |  | Enable Anthropic payload extraction and parsing |

#### `ebpf.payload_extraction.http.genai.openai`

| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.genai.openai.enabled` | `boolean` | `OTEL_EBPF_HTTP_OPENAI_ENABLED` |  |  |  | Enable OpenAI payload extraction and parsing |

#### `ebpf.payload_extraction.http.graphql`

| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.graphql.enabled` | `boolean` | `OTEL_EBPF_HTTP_GRAPHQL_ENABLED` |  |  |  | Enable GraphQL payload extraction and parsing |

#### `ebpf.payload_extraction.http.sqlpp`

| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.payload_extraction.http.sqlpp.enabled` | `boolean` | `OTEL_EBPF_HTTP_SQLPP_ENABLED` |  |  |  | Enable SQL++ payload extraction and parsing |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `ebpf.redis_db_cache.enabled` | `boolean` | `OTEL_EBPF_BPF_REDIS_DB_CACHE_ENABLED` |  |  |  |  |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `internal_metrics.prometheus.allow_service_graph_self_references` | `boolean` | `OTEL_EBPF_PROMETHEUS_ALLOW_SERVICE_GRAPH_SELF_REFERENCES` |  |  |  |  |
...
| `internal_metrics.prometheus.disable_build_info` | `boolean` | `OTEL_EBPF_PROMETHEUS_DISABLE_BUILD_INFO` |  |  |  |  |
...
| `internal_metrics.prometheus.port` | `integer` | `OTEL_EBPF_PROMETHEUS_PORT` |  |  |  |  |
...
| `javaagent.debug` | `boolean` | `OTEL_EBPF_JAVAAGENT_DEBUG` |  |  |  |  |
| `javaagent.debug_instrumentation` | `boolean` | `OTEL_EBPF_JAVAAGENT_DEBUG_INSTRUMENTATION` |  |  |  |  |
...
| `metrics.allow_service_graph_self_references` | `boolean` | `OTEL_EBPF_ALLOW_SERVICE_GRAPH_SELF_REFERENCES` |  |  |  |  |
...
| `metrics.insecure_skip_verify` | `boolean` | `OTEL_EBPF_INSECURE_SKIP_VERIFY` |  |  |  | Is not standard, so we don't follow the same naming convention |
...
| `network.enable` | `boolean` | `OTEL_EBPF_NETWORK_METRICS` |  |  | Yes | Network metrics. Default value is false (disabled)  add "network" or "network_inter_zone" to OTEL_EBPF_METRICS_FEATURES  TODO OBI 3.0: remove |
...
| `network.print_flows` | `boolean` | `OTEL_EBPF_NETWORK_PRINT_FLOWS` |  |  |  | The network flows in the Standard Output, if true |
...
| `network.sampling` | `integer` | `OTEL_EBPF_NETWORK_SAMPLING` |  |  |  | Holds the rate at which packets should be sampled and sent to the target collector. E.g. if set to 100, one out of 100 packets, on average, will be sent to the target collector. |
...
|---|---|---|---|---|---|---|
|  | `integer` | `OTEL_METRIC_EXPORT_INTERVAL` | `60000` |  |  | Supports metric intervals as specified by the standard OTEL definition. OTEL_EBPF_METRICS_INTERVAL takes precedence over it. |
| `otel_metrics_export.allow_service_graph_self_references` | `boolean` | `OTEL_EBPF_ALLOW_SERVICE_GRAPH_SELF_REFERENCES` |  |  |  |  |
...
| `otel_metrics_export.insecure_skip_verify` | `boolean` | `OTEL_EBPF_INSECURE_SKIP_VERIFY` |  |  |  | Is not standard, so we don't follow the same naming convention |
...
| `otel_traces_export.insecure_skip_verify` | `boolean` | `OTEL_EBPF_INSECURE_SKIP_VERIFY` |  |  |  | Is not standard, so we don't follow the same naming convention |
...
| YAML Path | Type | Env Var | Default | Values | Deprecated | Description |
|---|---|---|---|---|---|---|
| `prometheus_export.allow_service_graph_self_references` | `boolean` | `OTEL_EBPF_PROMETHEUS_ALLOW_SERVICE_GRAPH_SELF_REFERENCES` |  |  |  |  |
...
| `prometheus_export.disable_build_info` | `boolean` | `OTEL_EBPF_PROMETHEUS_DISABLE_BUILD_INFO` |  |  |  |  |
...
| `prometheus_export.port` | `integer` | `OTEL_EBPF_PROMETHEUS_PORT` |  |  |  |  |
...
| `stats.print_stats` | `boolean` | `OTEL_EBPF_STATS_PRINT_STATS` |  |  |  | The stats in the Standard Output, if true |
