...
| TCP Connection Summary | Node-wide statistical metric collection | Reports per-connection lifetime statistics at `tcp_close`: smoothed RTT, RTT mean deviation, total retransmissions, out-of-order packets, and segments sent/received. Exported as histograms for P50/P95/P99 aggregation. | Fires once per connection close; `src.port` may be `0` on the RST-receiver side (same limitation as TCP RTT). The `srtt_us` and `mdev_us` fields are scaled to real microseconds (`>>3` and `>>2` respectively). Connections with `srtt_us == 0` (no RTT sample) or exceeding `k_max_srtt_allowed` are dropped. |
