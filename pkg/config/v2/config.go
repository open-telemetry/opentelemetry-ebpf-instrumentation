...
package config // import "go.opentelemetry.io/obi/pkg/config/v2"
...
type Rule struct {
	...
	Refine      map[string]any `yaml:"refine"`
}
...
func ParseMap(raw map[string]any, mode DeploymentMode) (*Document, *Extension, error) {
	...
	if version, ok := nestedString(raw, "version"); ok {
		...
		for _, key := range []string{"policy", "rules", "instrumentation", "runtimes", "network", "limits", "engine", "safety", "channels", "telemetry"} {
...
func hasSection(cfg *Extension, section string) bool {
	switch section {
	case "enrich":
		return len(cfg.Enrich) > 0
	case "correlation":
		return len(cfg.Correlation) > 0
	case "daemon":
		return len(cfg.Daemon) > 0
	default:
		return false
	}
}
