...
// joinGroupV7ShortMetadata declares a protocols[0].metadata length covering only the
// first subscribed topic while the bytes of the second one follow: the parser must not
// read past the declared length.
func joinGroupV7ShortMetadata(groupID string, topics ...string) []byte {
	full := consumerSubscription(topics...)
	declared := Int16Len + Int32Len + Int16Len + len(topics[0])
	...
	b.compactArrayLen(1)
	b.compactStr("range")
	b.uvarint(uint64(declared + 1)) // metadata length: first topic only
	b.raw(full)                     // ... but all topics are on the wire
	b.tagged()
	b.tagged()
	return buildRequest(APIKeyJoinGroup, 7, true, b.buf)
}
...
func consumerGroupHeartbeatV0(groupID string, memberEpoch int32, subscribed []string, owned []UUID) []byte {
	b := &reqBuilder{}
	b.compactStr(groupID)
	b.compactStr("member-abc-123")
	b.i32(memberEpoch) // 0 = join, -1 = leave
...
func TestParseGroupRequest(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectProtocolType: "connect",
		},
		{
			// protocol types are free-form strings, not topic names
			name:               "join group v5 with a protocol type outside the topic charset",
			packet:             joinGroupV5("weird-group", "my protocol"),
			expectGroupID:      "weird-group",
