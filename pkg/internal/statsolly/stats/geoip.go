...
func geoiplog() *slog.Logger {
	return slog.With("component", "stats.GeoIP")
}

func GeoIPProvider(cfg *GeoIP, input, output *msg.Queue[[]*ebpf.Stat]) swarm.InstanceFunc {
		...
		return func(_ context.Context) {
			...
			for stats := range in {
				for _, stat := range stats {
					...
					stat.Attrs.Metadata[attr.SrcCountry] = srcInfo.Country
					stat.Attrs.Metadata[attr.DstCountry] = dstInfo.Country
					stat.Attrs.Metadata[attr.SrcASN] = srcInfo.ASN
					stat.Attrs.Metadata[attr.DstASN] = dstInfo.ASN
				}
				output.Send(stats)
			}
		}, nil
