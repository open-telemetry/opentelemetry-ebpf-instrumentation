...
func TestRuntimeMetricsReporterRecordsJVMClassMetrics(t *testing.T) {
	t.Run("zero unloaded classes", func(t *testing.T) {
		testJVMClassMetrics(t, 0)
	})
	t.Run("unloaded classes", func(t *testing.T) {
		testJVMClassMetrics(t, 5)
	})
}

func testJVMClassMetrics(t *testing.T, unloaded uint64) {
	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()
	...
	assert.Equal(t, int64(unloaded), got["jvm.class.unloaded"][0].Value)
