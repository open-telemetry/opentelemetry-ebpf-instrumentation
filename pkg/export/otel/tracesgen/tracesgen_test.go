...
func TestGroupSpans_ParentBasedSampling(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "traceidratio 1.0 with parent (should ignore parent)",
			samplerConfig: services.SamplerConfig{Name: "traceidratio", Arg: "1.0"},
			traceFlags:    0x00, // parent not sampled, but should be ignored
			hasParent:     true,
			expectSampled: true, // Should be sampled due to 1.0 ratio, ignoring parent
		},
		{
			name:          "always_on with non-sampled parent (should ignore parent)",
			samplerConfig: services.SamplerConfig{Name: "always_on", Arg: ""},
			traceFlags:    0x00, // parent not sampled, but should be ignored
			hasParent:     true,
			expectSampled: true, // Should always be sampled, ignoring parent
		},
	}
