...
func (m *meter) int64ObservableInstrument(id Instrument, callbacks []metric.Int64Callback) (int64Observable, error) {
	...
	return m.int64ObservableInsts.Lookup(key, func() (int64Observable, error) {
		inst := newInt64Observable(m, InstrumentKind(id.Kind), id.Name, id.Description, id.Unit)
		for _, insert := range m.int64Resolver.inserters {
			...
			in, rem, err := insert.Instrument(id, insert.readerDefaultAggregation(InstrumentKind(id.Kind)))
			if err != nil {
				return inst, err
			}
			// Drop aggregation
...
func (p int64InstProvider) histogramAggs(name string, cfg metric.Int64HistogramConfig) ([]aggregate.Measure[int64], []aggregate.Remove, error) {
	...
	return measures, removers, errors.Join(nil, err)
}
