...
type Reader interface {
	...
	// Reader methods.
	temporality(InstrumentKind) sdkmetricdata.Temporality

	// aggregation returns what Aggregation to use for an instrument kind.
	//
	// This method needs to be concurrent safe with itself and all the other
	// Reader methods.
	aggregation(InstrumentKind) sdkmetric.Aggregation // nolint:revive  // import-shadow for method scoped by type.
	...
	Collect(ctx context.Context, rm *sdkmetricdata.ResourceMetrics) error
...
type sdkProducer interface {
	...
	produce(context.Context, *sdkmetricdata.ResourceMetrics) error
}
...
type Producer interface {
	...
	Produce(context.Context) ([]sdkmetricdata.ScopeMetrics, error)
	// DO NOT CHANGE: any modification will not be backwards compatible and
	// must never be done outside of a new major release.
}
...
type produceHolder struct {
	produce func(context.Context, *sdkmetricdata.ResourceMetrics) error
}
...
func (p shutdownProducer) produce(context.Context, *sdkmetricdata.ResourceMetrics) error {
