...
package metric // import "go.opentelemetry.io/obi/pkg/export/otel/metric"
...
func (p *pipeline) produce(ctx context.Context, rm *sdkmetricdata.ResourceMetrics) error {
	p.Lock()
	defer p.Unlock()
...
func (i *inserter[N]) Instrument(inst Instrument, readerAggregation sdkmetric.Aggregation) ([]aggregate.Measure[N], []aggregate.Remove, error) {
	var (
		matched  bool
...
func (i *inserter[N]) readerDefaultAggregation(kind InstrumentKind) sdkmetric.Aggregation {
	aggregation := i.pipeline.reader.aggregation(kind)
	switch aggregation.(type) {
	case nil, sdkmetric.AggregationDefault:
...
// is returned.
func (i *inserter[N]) cachedAggregator(scope instrumentation.Scope, kind InstrumentKind, stream Stream, readerAggregation sdkmetric.Aggregation) (meas aggregate.Measure[N], remove aggregate.Remove, aggID uint64, err error) {
...
func (i *inserter[N]) aggregateFunc(b aggregate.Builder[N], agg sdkmetric.Aggregation, kind InstrumentKind) (meas aggregate.Measure[N], remove aggregate.Remove, comp aggregate.ComputeAggregation, err error) {
	...
	case sdkmetric.AggregationDrop:
		// Return nil in and out to signify the drop aggregator.
	case sdkmetric.AggregationLastValue:
		switch kind {
	...
	case sdkmetric.AggregationBase2ExponentialHistogram:
		var noSum bool
		switch kind {
		case InstrumentKindUpDownCounter, InstrumentKindObservableUpDownCounter, InstrumentKindObservableGauge, InstrumentKindGauge:
			// The sum should not be collected for any instrument that can make
...
// | Observable Gauge         | ✓    | ✓         |     | ✓         | ✓                     |.
func isAggregatorCompatible(kind InstrumentKind, agg sdkmetric.Aggregation) error {
	switch agg.(type) {
	case sdkmetric.AggregationDefault:
		return nil
	case sdkmetric.AggregationExplicitBucketHistogram, sdkmetric.AggregationBase2ExponentialHistogram:
		switch kind {
...
func (r resolver[N]) HistogramAggregators(id Instrument, boundaries []float64) ([]aggregate.Measure[N], []aggregate.Remove, error) {
	...
	for _, i := range r.inserters {
		...
		if histAgg, ok := agg.(sdkmetric.AggregationExplicitBucketHistogram); ok && len(boundaries) > 0 {
