...
func TestMetricsExpiration(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		if !strings.Contains(exported, `obi_network_flow_bytes_total{dst_name="bar",src_name="foo"} 123`) {
			return false
		}
		if !strings.Contains(exported, `obi_network_flow_bytes_total{dst_name="bae",src_name="baz"} 456`) {
			return false
		}
		return true
	...
	require.Eventually(t, func() bool {
		...
		if !strings.Contains(m, `obi_network_flow_bytes_total{dst_name="bar",src_name="foo"} 246`) {
			return false
		}
		exported = m
		return true
	...
	require.Eventually(t, func() bool {
		m := getMetrics(t, promURL)
		if !strings.Contains(m, `obi_network_flow_bytes_total{dst_name="bae",src_name="baz"} 456`) {
			return false
		}
		exported = m
		return true
