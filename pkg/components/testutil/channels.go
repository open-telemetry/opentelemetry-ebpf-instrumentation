...
// ReadChannelTimer tries to read a message from a channel and returns it. If there isn't any
// message after the provided timer expires, it fails the provided test.
// This method is useful for providing timers that are external to the test bubble when using
// synctest.Test.
func ReadChannelTimer[T any](t *testing.T, inCh <-chan T, timeoutTimer *time.Timer) T {
	t.Helper()
	var item T
	select {
	case item = <-inCh:
		return item
	case <-timeoutTimer.C:
		t.Fatalf("timeout while waiting for event in input channel")
	}
	return item
}
