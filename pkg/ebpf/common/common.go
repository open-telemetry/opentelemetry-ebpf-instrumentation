...
const (
	EventTypeHTTPRequest           = uint8(BpfEventTypeEVENT_HTTP_REQUEST)            // HTTP server request
	EventTypeGRPCRequest           = uint8(BpfEventTypeEVENT_GRPC_REQUEST)            // gRPC server request
	EventTypeHTTPClient            = uint8(BpfEventTypeEVENT_HTTP_CLIENT)             // HTTP client request
	EventTypeGRPCClient            = uint8(BpfEventTypeEVENT_GRPC_CLIENT)             // gRPC client request
	EventTypeSQL                   = uint8(BpfEventTypeEVENT_SQL_CLIENT)              // SQL client event
	EventTypeKHTTP                 = uint8(BpfEventTypeEVENT_K_HTTP_REQUEST)          // HTTP Events generated by kprobes
	EventTypeKHTTP2                = uint8(BpfEventTypeEVENT_K_HTTP2REQUEST)          // HTTP2/gRPC Events generated by kprobes
	EventTypeTCP                   = uint8(BpfEventTypeEVENT_TCP_REQUEST)             // Unknown TCP protocol to be classified by user space
	EventTypeGoSarama              = uint8(BpfEventTypeEVENT_GO_KAFKA)                // Kafka client for Go (Shopify/IBM Sarama)
	EventTypeGoRedis               = uint8(BpfEventTypeEVENT_GO_REDIS)                // Redis client for Go
	EventTypeGoKafkaGo             = uint8(BpfEventTypeEVENT_GO_KAFKA_SEG)            // Kafka-Go client from Segment-io
	EventTypeTCPLargeBuffer        = uint8(BpfEventTypeEVENT_TCP_LARGE_BUFFER)        // Dynamically sized TCP buffers
	EventOTelSDKGo                 = uint8(BpfEventTypeEVENT_GO_SPAN)                 // OTel SDK manual span
	EventTypeGoMongo               = uint8(BpfEventTypeEVENT_GO_MONGO)                // Go MongoDB spans
	EventTypeFailedConnect         = uint8(BpfEventTypeEVENT_FAILED_CONNECT)          // Failed Connections
	EventTypeDNS                   = uint8(BpfEventTypeEVENT_DNS_REQUEST)             // DNS events
	EventTypeGoRuntimeMetric       = uint8(BpfEventTypeEVENT_GO_RUNTIME_METRICS)      // Go runtime scalar metrics
	EventTypeGoChannelLink         = uint8(BpfEventTypeEVENT_GO_CHANNEL_LINK)         // Go channel handoff span links
	EventTypeJVMMemoryPoolGC       = uint8(BpfEventTypeEVENT_JVM_MEM_POOL_GC)         // JVM memory pool GC metrics
	EventTypeGoAutoSpan            = uint8(BpfEventTypeEVENT_GO_AUTO_SPAN)            // Go Auto SDK OTLP JSON span
	EventTypeGoRuntimeHistogram    = uint8(BpfEventTypeEVENT_GO_RUNTIME_HISTOGRAM)    // Go runtime histogram metrics
	EventTypeGoAutoActivated       = uint8(BpfEventTypeEVENT_GO_AUTO_ACTIVATED)       // internal Auto SDK activation control event
	EventTypeNodejsEventLoop       = uint8(BpfEventTypeEVENT_NODEJS_EVENTLOOP)        // Node.js event-loop runtime metrics
	EventNodeSpan                  = uint8(BpfEventTypeEVENT_NODE_SPAN)               // OTel API manual span from the Node.js span bridge
	EventTypeKHTTP2RequestHeaders  = uint8(BpfEventTypeEVENT_K_HTTP2REQUEST_HEADERS)  // Internal request HPACK observation
	EventTypeKHTTP2ResponseHeaders = uint8(BpfEventTypeEVENT_K_HTTP2RESPONSE_HEADERS) // Internal response HPACK observation
	EventTypeNodejsGC              = uint8(BpfEventTypeEVENT_NODEJS_GC)               // Node.js/V8 garbage-collection cycle
	EventTypeNodejsHeapSpace       = uint8(BpfEventTypeEVENT_NODEJS_HEAP_SPACE)       // Node.js/V8 heap-space sample
	EventTypePythonRuntimeMetric   = uint8(BpfEventTypeEVENT_PYTHON_RUNTIME_METRICS)  // EVENT_PYTHON_RUNTIME_METRICS - Python GC counters
	EventTypeJVMRuntimeMetrics     = uint8(BpfEventTypeEVENT_JVM_RUNTIME_METRICS)     // EVENT_JVM_RUNTIME_METRICS - JVM runtime metrics
)
