...
func maybeFastCGI(b *LargeBuffer) bool {
	...
	b.ResetRead()
	if b.Remaining() <= fastCGIRequestHeaderLen {
		return false
	}
	// Peek — non-advancing, so same buffer works for detectFastCGI.
	data, _ := b.Peek(b.Remaining())
	return bytes.Index(data, []byte(requestMethodKey)) >= 0
}
...
func parseHeader(b *LargeBuffer) ([]byte, error) {
	for {
		...
		// ReadN advances cursor past the header; zero-copy within chunk.
		hdrBytes, err := b.ReadN(fastCGIRequestHeaderLen)
		if err != nil {
			return nil, errors.New("payload too short")
		}
		...
		if hdr.Type == fcgiFrameTypeParams {
			...
			// ReadN(remaining) — materialise params payload; zero-copy single-chunk.
			rest, _ := b.ReadN(b.Remaining())
			return rest, nil
		}
		payloadOffset := int(hdr.ContentLength) + int(hdr.PaddingLength)
		if err := b.Skip(payloadOffset); err != nil {
			return nil, errors.New("payload too short")
		}
	}
}
...
func detectFastCGI(b, rb *LargeBuffer) (string, string, int) {
	...
	if methodPos >= 0 {
		...
		// ReadN(remaining) — materialise response once; zero-copy single-chunk.
		rbRaw, _ := rb.ReadN(rb.Remaining())
		if len(rbRaw) >= 2 {
