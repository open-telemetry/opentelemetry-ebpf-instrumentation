...
| `otel_traces_export.compression` | `string` | `OTEL_EXPORTER_OTLP_TRACES_COMPRESSION` |  |  |  | Is the compression applied to exported OTLP payloads. Accepted values are gzip, zlib, deflate, snappy, x-snappy-framed, zstd, lz4 and none. Left unset, the OTLP exporter's own default of gzip applies. |
