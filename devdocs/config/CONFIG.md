...
| `otel_traces_export.compression` | `string` | `OTEL_EXPORTER_OTLP_TRACES_COMPRESSION` |  | `deflate`, `gzip`, `lz4`, `none`, `snappy`, `x-snappy-framed`, `zlib`, `zstd` |  | Is the compression applied to exported OTLP payloads. Left unset, the OTLP exporter's own default of gzip applies. gRPC accepts a narrower set than HTTP, which Validate checks once the protocol is known. |
