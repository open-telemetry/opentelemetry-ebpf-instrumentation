...
GC heap-summary and memory-pool events; additional JVM runtime areas would need
separate probe support.

## Metrics

| OTel metric | Prometheus metric | Runtime source | Export behavior |
| --- | --- | --- | --- |
| `jvm.memory.used` | `jvm_memory_used_bytes` | HotSpot `GCTracer::report_gc_heap_summary` and `hotspot:mem__pool__gc__*` USDT probes | Emits whole-heap usage with `jvm.memory.type=heap` and per-pool usage with `jvm.memory.pool.name`. |
...
OBI emits standard JVM memory metric names. Heap-summary values are represented
as `jvm.memory.used` with `jvm.memory.type=heap` rather than an OBI-specific heap
metric.
...
1. During Java process discovery, userspace attaches a uprobe to
   `GCTracer::report_gc_heap_summary` and USDT probes to
...
Snapshots update when HotSpot emits GC heap-summary or memory-pool GC probe
events. A newly started Java process emits JVM runtime metrics after the relevant
HotSpot GC probes fire.
