...
`detect_h2` is resumable across tail calls via `tailcall_ctx.h2_scan_pos`. After `write_h2_tp` injects HPACK into a frame, it tail-calls `detect_h2` with `scan_pos` past the just-injected frame so multiplexed senders that batch multiple HEADERS frames into one `sendmsg` (Node grpc-js, Go loopyWriter under contention) get every stream injected. The packet cap is four frames. The HPACK scanner processes 15 parser steps per verifier-safe program and authenticates each self-continuation against the saved parser state. A per-packet counter reserves the two-call entry prefix and stops expensive mixed batches before the kernel's 33-tail-call limit; four single-pass frames cost 25 calls and remain supported.
...
The scan recognizes the encodings that put identifying bytes on the wire: a literal name in any of the three literal prefixes, plain or huffman, and a dyn-table name reference whose value is plain (`0x37` + `00-` + dash positions). The sk_msg path has no connection-persistent HPACK mirror, so it resolves a dynamic name only when that name was inserted earlier in the same bounded block and lazy eviction accounting proves the entry is still active. A pre-block reference or a fully indexed traceparent remains unknown and suppresses injection.
...
- **kprobe HPACK parser** (`http2_grpc_start`, SERVER side): reconstructs HEADERS/CONTINUATION blocks across callbacks under an exact connection-generation lease, then mirrors the connection's HPACK dynamic table before selecting a parent. This handles indexed names and fully indexed traceparent values on persistent multiplexed connections without relying on a connection-wide `trace_map`. PADDED/PRIORITY prefixes are removed before parsing, ambiguous or truncated table state fails closed, and the bounded parser/validator stages tail-call directly into the shared server commit program (jump-table slot 14).
