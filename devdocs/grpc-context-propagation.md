...
The direct Go injectors retain per-write state when a HEADERS frame does not carry `END_HEADERS` and append the OBI field to the terminal CONTINUATION fragment. This covers `net/http`, `x/net/http2`, and grpc-go when the terminal frame and writer have room for the complete field. If that capacity is unavailable, direct injection makes no mutation. The plaintext `sk_msg` fallback intentionally injects only complete HEADERS frames; fragmented blocks are left unchanged because a header block can cross socket-message boundaries and the socket path has no failure-safe buffering contract for that sequence.
