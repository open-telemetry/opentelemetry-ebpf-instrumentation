...
The direct Go injectors retain per-write state when a HEADERS frame does not carry `END_HEADERS` and append the OBI field to the terminal CONTINUATION fragment. The largest observed nonterminal fragment provides a conservative lower bound for the peer's frame-size limit, so `net/http`, `x/net/http2`, and grpc-go can append only when the enlarged terminal fragment remains within that bound and the writer has capacity for the complete field. Before any nonterminal fragment establishes a bound, direct injection uses the HTTP/2 minimum frame size; a complete frame larger than 16 KiB is therefore left unchanged because the writer boundary does not expose the negotiated limit. The plaintext `sk_msg` fallback intentionally injects only complete HEADERS frames; fragmented blocks are left unchanged because a header block can cross socket-message boundaries and the socket path has no failure-safe buffering contract for that sequence.
