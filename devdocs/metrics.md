...
Resource attributes exported through Prometheus `target_info` / `traces_target_info` and the corresponding OTLP resources can be filtered with `attributes.select`. By default, detected resource attributes are preserved. For example:
