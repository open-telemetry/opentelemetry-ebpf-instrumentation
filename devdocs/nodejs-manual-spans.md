...
string/number/bool), `events` (names only), `scope` (instrumentation scope
name). Budgets, chosen to mirror the Go path and fit the payload buffer:
name ≤ 128 chars, ≤ 16 attributes, keys ≤ 64, string values ≤ 256, whole
payload ≤ 1900 bytes (attributes/events are dropped first on overflow).
...
- **End-time context sampling.** The request context is read when the span
  *ends*. A manual span that outlives its request falls back to the bridge
  ...
  can split across trace IDs.
- **Payload budgets** (above). Span events carry names only; links,
  non-primitive attribute values and `traceState` are not forwarded.
