...
func assertHTTPRequests(t *testing.T, comm, urlPath string) {
	...
	test.Eventually(t, testTimeout, func(t require.TestingT) {
		...
		assert.LessOrEqual(t, len(traces), 1)
...
func assertSQLOperationErrored(t *testing.T, comm, op, table, db string) {
	...
	test.Eventually(t, testTimeout, func(t require.TestingT) {
		...
		// assert.True(t, found)
		// assert.Equal(t, "TODO: broken", tag.Value) // TODO(matt): fix this once the SSL path is fixed
	}, test.Interval(100*time.Millisecond))
}
...
func testPythonMySQL(t *testing.T) {
	...
	// TODO(matt): mysql connection in tests happens over SSL, and when using SSL,
	// the response buffer sent with the TCP event is long 4 bytes. Investigate and fix this.
