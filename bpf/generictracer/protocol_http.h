...
__obi_protocol_http(struct pt_regs *ctx, unsigned char *(*tp_loop_fn)(unsigned char *, const u16)) {
    ...
    if (args->packet_type == PACKET_TYPE_REQUEST && (info->status == 0) &&
        ...
        if (g_bpf_traceparent_enabled && capture_header_buffer &&
            ...
            // acceptable degradation since all large buffers are keyed on
            // original_trace_id regardless of later trace ID updates.
        }
        // TP parsing not needed or tail-call failed: emit large buffer with
        // current (stable) trace ID.
        http_send_large_buffer(info,
                               (void *)args->u_buf,
                               args->bytes_len,
                               args->packet_type,
