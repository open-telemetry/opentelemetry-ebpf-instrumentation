            ...
            if (!ssl) {
                ...
                if (buf) {
                    ...
                    handle_buf_with_connection(
                        ctx, &s_args.p_conn, 0, buf, size, NO_SSL, TCP_SEND, orig_dport);
                }
            } else {
    ...
    if (success) {
        ...
        if (is_tcp_socket_never_connected(sk)) {
            ...
            if (ct && !ct->established && !ct->failed) {
                ...
                failed_to_connect_event(&info, 0, orig_dport, ct->ts);
            }
        }
...
int BPF_KPROBE(obi_kprobe_sock_def_error_report, struct sock *sk) {
    ...
    if (parse_sock_info(sk, &info.conn)) {
        ...
        if (args) {
            if (!args->failed) {
                dbg_print_http_connection_info(&info.conn);
                failed_to_connect_event(&info, 0, orig_dport, args->ts);
            ...
            if (conn_pid && conn_pid->id == id) {
                dbg_print_http_connection_info(&info.conn);
                failed_to_connect_event(&info, 0, orig_dport, conn_pid->ts);
            }
        }
    }
    ...
    if (parse_sock_info((struct sock *)sock_ptr, &info.conn)) {
        ...
        if (!ssl) {
            ...
            if (buf && copied_len) {
                ...
                handle_buf_with_connection(
                    ctx, &info, 0, buf, copied_len, NO_SSL, TCP_RECV, orig_dport);
            }
        } else {
