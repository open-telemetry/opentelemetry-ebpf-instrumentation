...
} traces_ctx_flags SEC(".maps");

static __always_inline obi_ctx_info_t *obi_ctx__get(const u64 pid_tgid) {
    return bpf_map_lookup_elem(&traces_ctx_v1, &pid_tgid);
}

static __always_inline u8 *obi_ctx__get_flags(const u64 pid_tgid) {
    return bpf_map_lookup_elem(&traces_ctx_flags, &pid_tgid);
}
...
static __always_inline long obi_ctx__set(const u64 pid_tgid, const tp_info_t *info) {
    ...
    const long ret = bpf_map_update_elem(&traces_ctx_v1, &pid_tgid, &obi_info, BPF_ANY);
    if (ret != 0) {
        return ret;
    }
    return bpf_map_update_elem(&traces_ctx_flags, &pid_tgid, &info->flags, BPF_ANY);
}
...
static __always_inline long obi_ctx__del(const u64 pid_tgid) {
    bpf_map_delete_elem(&traces_ctx_flags, &pid_tgid);
