...
static void test_overflow_counts_each_span_once(void) {
    ...
    check_u64(1, stack()->overflow[k_obi_ctx_sql], "fifth span is counted, not stored");
    ...
    check_u64(
        1, stack()->overflow[k_obi_ctx_sql], "restart of an unstored span is not counted again");
    begin(k_obi_ctx_sql, span(17), 250);
    check_u64(2, stack()->overflow[k_obi_ctx_sql], "a deeper unstored span is counted");
...
static void test_sibling_after_unstored_end_is_counted(void) {
    ...
    check_u64(2, stack()->overflow[k_obi_ctx_sql], "two unstored spans");

    end(k_obi_ctx_sql, span(23));
    // a second query at the same depth as the one that just ended
    begin(k_obi_ctx_sql, span(24), 250);
    check_u64(2, stack()->overflow[k_obi_ctx_sql], "a sibling after an end is a new span");
    end(k_obi_ctx_sql, span(24));
    end(k_obi_ctx_sql, span(22));
    check_u64(0, stack()->overflow[k_obi_ctx_sql], "all unstored spans ended");
...
static void test_unrelated_end_keeps_overflow_accounting(void) {
    reset();
    begin(k_obi_ctx_http_server, span(27), 100);
    begin(k_obi_ctx_sql, span(28), 210);
    begin(k_obi_ctx_sql, span(29), 220);
    begin(k_obi_ctx_sql, span(30), 230);
    begin(k_obi_ctx_sql, span(31), 240);
    check_u64(1, stack()->overflow[k_obi_ctx_sql], "fifth span overflows");

    // ClientConn.Close while overflowed: no gRPC client span exists here
    end_unknown(k_obi_ctx_grpc_client);
    ...
    check_u64(1, stack()->overflow[k_obi_ctx_sql], "the restart marker survived the unrelated end");
    check_u64(32, thread_span(), "the restarted span is current");

    end_unknown(k_obi_ctx_sql);
    check_u64(0, stack()->overflow[k_obi_ctx_sql], "the unstored span's own end consumes it");
    check_u64(k_obi_ctx_max_depth, stack()->depth, "the stored frames are intact");
    check_u64(30, thread_span(), "the deepest stored span is current again");

    end(k_obi_ctx_sql, span(30));
    end(k_obi_ctx_sql, span(29));
    end(k_obi_ctx_sql, span(28));
    check_u64(27, thread_span(), "the server span is restored");
}
