...
int obi_uprobe_readRequestStart(struct pt_regs *ctx) {
    ...
    if (!existing || (existing->d_port == 0 && existing->s_port == 0)) {
        ...
        if (c_ptr) {
            void *conn_conn_ptr =
                c_ptr + k_go_iface_data_offset + go_offset_of(ot, (go_offset){.v = _c_rwc_pos}); // embedded struct
            void *tls_state = 0;
...
int obi_uprobe_writeSubset(struct pt_regs *ctx) {
    ...
    s64 size = 0;
    bpf_probe_read(
        &size, sizeof(s64), (void *)(io_writer_addr + io_writer_buf_ptr_pos + k_go_slice_len_offset)); // grab size
...
int obi_uprobe_http2serverConn_runHandler(struct pt_regs *ctx) {
    ...
    if (sc) {
        void *conn_ptr = 0;
        bpf_probe_read(
            &conn_ptr, sizeof(void *), sc + go_offset_of(ot, (go_offset){.v = _sc_conn_pos}) + k_go_iface_data_offset);
        bpf_dbg_printk("conn_ptr=%llx", conn_ptr);
        if (conn_ptr) {
...
static __always_inline void setup_http2_client_conn(void *goroutine_addr,
    ...
    if (cc_ptr) {
        ...
        bpf_probe_read(&tconn, sizeof(tconn), (void *)(cc_ptr + cc_tconn_pos + k_go_iface_data_offset));
        bpf_dbg_printk("tconn=%llx", tconn);

        if (tconn) {
            void *tconn_conn = 0;
            bpf_probe_read(&tconn_conn, sizeof(tconn_conn), (void *)(tconn + k_go_iface_data_offset));
...
int obi_uprobe_http2FramerWriteHeaders_returns(struct pt_regs *ctx) {
    ...
    if (f_info) {
        ...
        bpf_probe_read(&w_ptr, sizeof(w_ptr), (void *)(f_info->framer_ptr + framer_w_pos + k_go_iface_data_offset));
...
int obi_uprobe_persistConnRoundTrip(struct pt_regs *ctx) {
    ...
    if (pc_ptr) {
        void *conn_conn_ptr =
            pc_ptr + k_go_iface_data_offset + go_offset_of(ot, (go_offset){.v = _pc_conn_pos}); // embedded struct
        void *tls_state = 0;
        bpf_probe_read(
            &tls_state,
            sizeof(tls_state),
