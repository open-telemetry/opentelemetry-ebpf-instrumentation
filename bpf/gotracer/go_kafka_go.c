    ...
    if (p_ptr) {
        ...
        if (topic_ptr) {
            ...
            if (trace) {
                ...
                void *conn_ptr = 0;
                bpf_probe_read(
                    &conn_ptr, sizeof(conn_ptr), (void *)(p_ptr->conn_ptr + k_go_iface_data_offset)); // find conn
...
int obi_uprobe_reader_read(struct pt_regs *ctx) {
    ...
    if (r_ptr) {
        ...
        if (conn) {
            void *conn_ptr = 0;
            bpf_probe_read(&conn_ptr, sizeof(conn_ptr), (void *)(conn + k_go_iface_data_offset)); // find conn
            bpf_dbg_printk("conn_ptr=%llx", conn_ptr);
            if (conn_ptr) {
