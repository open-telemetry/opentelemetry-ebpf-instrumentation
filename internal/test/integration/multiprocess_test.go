...
func TestMultiProcess(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		return true
	}, testTimeout, 500*time.Millisecond, "Failed to GET dont-instrument endpoint")
...
func checkReportedOnlyOnce(t *testing.T, baseURL, serviceName string) {
	...
	require.Eventually(t, func() bool {
		...
		if totalPromCount(t, results) != 3 {
			return false
		}
		return true
	}, testTimeout, 1000*time.Millisecond, "Prometheus query results not as expected")
}
...
func checkInstrumentedProcessesMetric(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		return true
	}, testTimeout, 1000*time.Millisecond, "Instrumented processes metrics not found")
}
