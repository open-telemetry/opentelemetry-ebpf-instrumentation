...
const manualSDKLoggerMessage = "this is a json log after sql query inside a manual sdk span"
...
func main() {
	...
	http.HandleFunc("/json_logger_manual_sdk", func(w http.ResponseWriter, _ *http.Request) {
		tracer := otel.Tracer("go_sql_logger")
		_, span := tracer.Start(context.Background(), "sig")
		...
		entry := map[string]any{
			"message": manualSDKLoggerMessage,
			"level":   "INFO",
			"ts":      time.Now().UTC().Format(time.RFC3339),
		}
		b, err := json.Marshal(entry)
