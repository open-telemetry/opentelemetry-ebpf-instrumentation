		...
		Assess("it sends decorated traces for the cronjob",
			func(ctx context.Context, t *testing.T, _ *envconf.Config) context.Context {
				require.Eventually(t, func() bool {
					...
					return len(sd) == 3 && sd[0].ErrType == jaeger.ErrTypeMissing && sd[1].ErrType == jaeger.ErrTypeMissing && sd[2].ErrType == jaeger.ErrTypeMissing
				}, testTimeout, 100*time.Millisecond, "waiting for cronjob traces with metadata")
				return ctx
			},
		).Feature()
