...
func runKafkaTestCase(t *testing.T, testCase TestCase) {
	...
	// consumer group on "process" spans is learned from the group-coordination requests
	// (JoinGroup, Heartbeat, OffsetCommit) the consumer sends on its coordinator
	...
	// (~3s) or OffsetCommit (~5s). Waiting for one matching span covers that warm-up.
